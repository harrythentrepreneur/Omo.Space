"""Generated Modal contract runtime for meta-ads-audit.
...
APP_NAME = 'cognition-meta-ads-audit'
WORKFLOW_VERSION = 'meta-ads-audit@0.1.0'
...
LIVE_MAX_TOKENS = 2600
LIVE_TEMPERATURE = 0.1
LIVE_TIMEOUT_SECONDS = 120
LIVE_INPUT_RATE_PER_MILLION = 0.14
LIVE_OUTPUT_RATE_PER_MILLION = 0.42
LIVE_MODEL_OUTPUT_SCHEMA = {'additionalProperties': False, 'properties': {'diagnoses': {'items': {'description': 'A diagnosis grounded in observations.', 'maxLength': 500, 'minLength': 8, 'title': 'Diagnosis', 'type': 'string'}, 'maxItems': 6, 'minItems': 1, 'title': 'Diagnoses', 'type': 'array'}, 'draft_changes': {'items': {'description': 'A proposed change only; never an applied mutation.', 'maxLength': 400, 'minLength': 8, 'title': 'Draft change', 'type': 'string'}, 'maxItems': 5, 'minItems': 0, 'title': 'Draft changes', 'type': 'array'}, 'evidence_coverage': {'description': 'How complete the supplied evidence is.', 'enum': ['complete', 'partial', 'limited'], 'title': 'Evidence coverage'}, 'guardrails': {'items': {'description': 'A policy or measurement guardrail.', 'maxLength': 400, 'minLength': 8, 'title': 'Guardrail', 'type': 'string'}, 'maxItems': 6, 'minItems': 1, 'title': 'Guardrails', 'type': 'array'}, 'missing_inputs': {'items': {'description': 'Evidence needed to increase confidence.', 'maxLength': 300, 'minLength': 3, 'title': 'Missing input', 'type': 'string'}, 'maxItems': 8, 'minItems': 0, 'title': 'Missing inputs', 'type': 'array'}, 'observations': {'items': {'additionalProperties': False, 'properties': {'area': {'description': 'Reviewed audit area.', 'enum': ['measurement', 'attribution', 'creative', 'audience', 'placements', 'budget', 'policy', 'structure'], 'title': 'Area'}, 'confidence': {'description': 'Confidence based on supplied evidence.', 'enum': ['high', 'medium', 'low'], 'title': 'Confidence'}, 'evidence': {'description': 'Input evidence supporting the finding.', 'maxLength': 700, 'minLength': 3, 'title': 'Evidence', 'type': 'string'}, 'finding': {'description': 'Evidence-grounded observation.', 'maxLength': 500, 'minLength': 8, 'title': 'Finding', 'type': 'string'}}, 'required': ['area', 'finding', 'evidence', 'confidence'], 'title': 'Audit observation', 'type': 'object'}, 'maxItems': 8, 'minItems': 1, 'title': 'Observations', 'type': 'array'}, 'platform_health': {'description': 'Plain-language overall state.', 'maxLength': 700, 'minLength': 8, 'title': 'Platform health', 'type': 'string'}, 'recommendations': {'items': {'additionalProperties': False, 'properties': {'next_check': {'description': 'What to verify next.', 'maxLength': 300, 'minLength': 8, 'title': 'Next check', 'type': 'string'}, 'owner': {'description': 'Suggested owner.', 'maxLength': 120, 'minLength': 3, 'title': 'Owner', 'type': 'string'}, 'priority': {'description': 'Order of attention.', 'enum': ['high', 'medium', 'low'], 'title': 'Priority'}, 'rationale': {'description': 'Why this follows from the evidence.', 'maxLength': 600, 'minLength': 8, 'title': 'Rationale', 'type': 'string'}, 'recommendation': {'description': 'A bounded recommended next move.', 'maxLength': 500, 'minLength': 8, 'title': 'Recommendation', 'type': 'string'}}, 'required': ['priority', 'recommendation', 'rationale', 'owner', 'next_check'], 'title': 'Prioritized recommendation', 'type': 'object'}, 'maxItems': 6, 'minItems': 1, 'title': 'Recommendations', 'type': 'array'}}, 'required': ['platform_health', 'evidence_coverage', 'observations', 'diagnoses', 'recommendations', 'draft_changes', 'missing_inputs', 'guardrails'], 'title': 'Meta Ads Audit provider result', 'type': 'object'}
...
def create_fastapi_app(
    ...
    *,
    ...
    web = FastAPI(title='meta-ads-audit', version='0.1.0')
