...
Generated Modal candidate for `phoneme-counter`. The source skill is vendored
at `source/SKILL.md` (SHA-256 `4ab30f0ffa0041f2d232d38fec59fa31d4be8fd90c7f288778bd9c09de4d5966`). Generated files must be changed
...
python3 packages/skill-to-modal/compiler.py \
  containers/phoneme-counter/source/SKILL.md \
  --profile packages/skill-to-modal/profiles/phoneme-counter.json \
  --out containers/phoneme-counter
python3 -m pytest -q -p no:cacheprovider containers/phoneme-counter/tests/test_contract.py
```

Deploy after the named Modal secret exists and the offline tests pass:

```bash
modal deploy containers/phoneme-counter/modal_app.py
```
