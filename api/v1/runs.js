...
function validateRunInvariants(body){
 if(body.status==='succeeded'){
  ...
  const outputIssues=validateSchema(outputSchema,body.result,'/result');if(outputIssues.length)return outputIssues;
...
function validateRequest(body){
 ...
 if(body.workflow.slug!==manifest.slug||(body.workflow.version!==undefined&&body.workflow.version!==manifest.workflow_version))return [issue('/workflow','unknown_workflow','Workflow is not available')];
 return validateSchema(inputSchema,body.input,'/input');
}
function strictProviderResult(data){
 if(data?.status!=='completed'||!validObject(data.result))return null;
 return validateSchema(outputSchema,data.result,'/result').length?null:data.result;
}
function publicStatus(row){
 if(row.execution_status==='succeeded')return 'succeeded';
 if(row.execution_status==='failed')return 'failed';
 if(row.execution_status==='dispatching')return 'running';
 return 'queued';
}
...
async function dispatchRun(store,row,env,fetchImpl){
 ...
 try{response=await fetchImpl(env.MODAL_UGC_ENDPOINT,{method:'POST',headers:{'content-type':'application/json','modal-key':env.MODAL_PROXY_TOKEN_ID,'modal-secret':env.MODAL_PROXY_TOKEN_SECRET,'x-cognition-run-id':row.run_id},body:JSON.stringify(row.input)})}catch{}
 ...
 const result=strictProviderResult(data);
 if(result){await store.succeed(row.run_id,owner,result);return}
 if(response?.ok&&data){code='INVALID_PROVIDER_OUTPUT';message='Workflow returned invalid output';retryable=false}
...
export function createMemoryServices({balanceCents=500}={}){
 ...
 const store={
  ...
  async due(limit){return [...byId.values()].filter(r=>!['succeeded','failed'].includes(r.execution_status)&&(!r.dispatch_lease_expires_at||r.dispatch_lease_expires_at<=now())).slice(0,limit)},
...
export async function createNeonStore(env){
 ...
 return {
  ...
  async lease(runId,owner,expires){const r=await pool.query("UPDATE run_requests SET execution_status='dispatching',dispatch_owner=$1,dispatch_lease_expires_at=$2,attempt_count=attempt_count+1,dispatched_at=$3,updated_at=$3 WHERE run_id=$4 AND execution_status IN ('queued','dispatching') AND (dispatch_lease_expires_at IS NULL OR dispatch_lease_expires_at <= $3) RETURNING *",[owner,expires,now(),runId]);return normalizeRow(r.rows[0]||null)},
  async due(limit){const r=await pool.query("SELECT * FROM run_requests WHERE execution_status IN ('claimed','queued','dispatching') AND (dispatch_lease_expires_at IS NULL OR dispatch_lease_expires_at <= $1) ORDER BY updated_at LIMIT $2",[now(),limit]);return r.rows.map(normalizeRow)},
...
export async function handleRun(req,res,env=process.env,fetchImpl=fetch,deps={}){
 if(!env.NEON_DATABASE_URL||!env.MODAL_UGC_ENDPOINT||!env.MODAL_PROXY_TOKEN_ID||!env.MODAL_PROXY_TOKEN_SECRET)return send(res,503,publicError('SERVER_NOT_CONFIGURED','Run service is not configured',true));
 ...
 const requestHash=hash(stable(req.body));const timestamp=now();const row={run_id:`run_${crypto.randomUUID().replaceAll('-','')}`,user_id:identity.userId,request_hash:requestHash,slug:manifest.slug,workflow_version:manifest.workflow_version,cost_cents:COST_CENTS,execution_status:'claimed',billing_status:'unbilled',input:req.body.input,created_at:timestamp,updated_at:timestamp,attempt_count:0,artifacts:[]};
 ...
 if(!await store.reserve(identity.userId,COST_CENTS,active.run_id)){await store.paymentFailed(active,{code:'PAYMENT_REQUIRED',message:'Insufficient balance',retryable:false,issues:[]});return send(res,402,runResource(active))}
