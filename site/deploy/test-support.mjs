...
assert.match(worker, /user_guest_/);
assert.match(worker, /cf-connecting-ip/);
assert.match(worker, /PAGE CONTEXT \(untrusted\)/);
...
assert.doesNotMatch(client, /innerHTML\s*=\s*text/);
assert.match(client, /Promise\.resolve\(''\)/);
assert.match(client, /context: 'Page: '/);
