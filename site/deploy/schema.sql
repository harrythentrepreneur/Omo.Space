...
-- Registry-first marketplace catalog. This is an additive shadow registry in
-- Phase 0: the existing 24 live listings continue through their current Worker
-- path while draft/review tools remain inactive and non-chargeable.
--
-- JSONB is intentional: Neon is authoritative for the registry. The rest of
-- this file remains D1-compatible for the existing credits fallback.
CREATE TABLE IF NOT EXISTS tools (
  ...
  status          TEXT NOT NULL CHECK (status IN ('draft', 'review', 'live', 'disabled')),
  ...
  CHECK (NOT chargeable OR status = 'live'),
  CHECK (NOT active OR status = 'live')
);

CREATE INDEX IF NOT EXISTS idx_tools_status_updated
  ON tools (status, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_tools_tier_status
  ON tools (tier, status);

CREATE INDEX IF NOT EXISTS idx_tools_owner_status
  ON tools (owner_id, status, updated_at DESC);
-- registry:tools:end
