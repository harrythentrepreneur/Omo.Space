...
- Universal workflow runtime release: PR #199 merged at `86361b0cfc67e5f6db805a2d90e7816b2121919a`; PR #200 aligned the Modal builder and production Worker selectors, both were deployed, and direct/same-origin route probes passed. The first real owner upload reached Modal as submission `sub_6384a14dda857e9a9a0429efb41dd53b` / dispatch `dispatch_ca31603bc8e000bc61ed4bc1b3e693e8` but failed in the `hermes` startup stage before compiler/release work because the dedicated Modal Nous invoke JWT was expired. A local renewable-auth repair now keeps the Portal refresh credential in a root-only persistent Modal auth store, mints a bounded short-lived invoke JWT in the trusted parent, gives the untrusted Hermes UID only that invoke JWT, and maps preparation failures to `hermes_auth_failed`; security review found no critical exposure and its three important findings were fixed. Host-skill tests pass 342/342; no replacement secret, push, deploy, or retry has occurred.
- Next: (1) create a dedicated Portal OAuth identity for the Modal builder, provision only its minimal refresh seed after owner authentication, then PR/review/merge/deploy the renewable-auth repair and run one exact Retry; (2) correlate the retry through dispatch, Hermes, compiler, GitHub release and marketplace run; (3) coordinator pushes the local loader/catalog/listing plus corrective-fix commits and verifies the 14 production cards; (4) obtain Harry's explicit specific approval for the isolated Japanese Modal deploy and canary; (5) re-run an authenticated top-up + Woven checkout canary to confirm the live payment loop; (6) move Omo onto its own Stripe account; (7) rotate the live sk exposed in chat.
- Blockers: the universal canary is blocked on a dedicated builder Portal OAuth login and production approval for secret provisioning/push/deploy; do not Retry while the deployed builder still uses the expired static token. The public static loader activation awaits the coordinator-owned push. Japanese Style Story Video's regenerated app separately awaits its exact live-change approval; Stripe LIVE runs on the shared PhonicsMaker Stripe account; a live sk was exposed in chat (rotation pending); the exact active $0.99 PhonicsMaker book slug is not identified and the local magic-link release is not deployed; existing signed-in account for a payment canary; the shared `omo-llm-runner` and Tier-1 runner; paid download fulfillment.
