...
- Creator builder infrastructure: issue #63 moved creator-build dispatch off Hetzner through Cloudflare cron -> protected Modal endpoint -> Worker-authoritative atomic claim -> one revision/source-bound ephemeral Hermes build. Issues #65/#67/#69 added recovery, safe failure stages, and immutable-checkout imports. PR #72's approvals-off approach was contained and reverted by PR #73 after review found terminal + GH_TOKEN unsafe. Issue #71 now uses technical privilege separation: untrusted Hermes has file/skills only and receives neither GH_TOKEN nor BUILD_WORKER_TOKEN; after Hermes exits, the trusted fixed-repo/base release adapter runs validation/tests and prepares only an issue/branch/PR with Harry's token. Local gates pass 156 Python + 187 Worker contracts; independent review and live canary remain required.
