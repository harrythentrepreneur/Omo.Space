...
- NEXT CONCRETE BUILD STEP: build and test the dedicated `phonicsmaker_core` Omo adapter around the preserved source layer, starting with the full book submit/poll/artifact path. The adapter must carry the real book inputs and all output variants before any listing is considered hosted.
