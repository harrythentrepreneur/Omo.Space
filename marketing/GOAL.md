...
- Creator builder infrastructure: issue #63 is moving creator-build dispatch off Hetzner. The implemented candidate is Cloudflare cron -> protected Modal endpoint -> Worker-authoritative atomic claim -> one revision/source-bound ephemeral Hermes build. Modal endpoint authentication and malformed-input behavior are verified; release/canary evidence is still pending.
