...
- Education hosting gate: Gate 1R proved OpenCode Go accepts `json_object`, regenerated the nine 0.2.1 runtimes, and reached 27/27 schema-valid outputs with 18/27 semantic passes at USD 0.00478016; authenticated proxy preflight was 2/2 HTTP 200. The acceptance gate still fails because digraph-spotter scored 0/3 semantic on absolute spans/coverage, so Modal deployment, marketplace registration, activation, and hosted canaries were not attempted. Every case and exact failure is recorded in `research/one-shot-rounds.md`.
...
Implement deterministic absolute-span normalization for digraph-spotter, add
the matching semantic validation before the single retry, and rerun the exact
same 27 provider cases. Do not deploy or activate the education wave until every
tool has at least one semantic pass.
