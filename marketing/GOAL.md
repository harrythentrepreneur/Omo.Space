...
This tick (bind-path-hardening, 2026-08-16T18:3xZ): origin/main 0 behind / 19 ahead (all ours; the +1 is the candidate-draft commit 18ceaff). Verified the candidate is bind-ready by probing the REAL profile through the compiler selector — found and fixed two selector gaps the fixture suite missed (book-field rule now long-form-aware; selector reads output_schema + model_output_schema merged because decodability is runtime-synthesized), updated the two vocabulary fixture constructions to own output_schema, and shipped `research/bind-candidate-vocabulary.py` (dry-run verified; --apply gated on Harry's recorded approval). Vocabulary+pin subset 4/4; full-suite failures byte-identical to baseline (8 pre-existing env failures in this cron session vs the 57/58 recorded in a different env); probe fires whole_book_vocabulary on the real profile with the bind patch. Status: WAITING on the same four Harry/coordinator-owned gates; metrics unchanged 0/0/0. No bind, regen, spend, send, deploy, push, or secret access occurred.
