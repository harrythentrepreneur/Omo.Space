...
- Trusted finalizer eligibility repair: issue #308 reproduced the production `invalid_finalizer_eligibility_count` as repeated exact `(published_slug, source_sha256)` submissions: release PRs #287 and #305 carry the same label-sorter source hash, so the unranked eligibility query returned stale and current rows. The local fix elects the newest exact-target submission by `updated_at`/`id` in eligibility and in the atomic Neon/D1/mock claim path, so older repeated rows cannot become a second generation or duplicate deployment/billing effect; Neon retains `FOR UPDATE SKIP LOCKED`, and D1 rechecks newest election plus selected `updated_at` in its update CAS. Regressions are RED-before/GREEN-after; router 310/310 and focused finalizer 169/169 pass. No deploy, production mutation, billing action, or finalizer retry has occurred.
- Next: complete exact-diff review, commit, open the focused #308 PR, and require protected CI; do not merge or deploy without separate approval.
- Blockers: PR #209 merged the independently approved Modal video-renderer fixture correction at `b8419d7a8fe12a26b09a7b5590bd1b40dfd8ad83`. Its identical Modal image proof returned 0 with 398 passed, 1 skipped, 14 unavailable-fixture tests deselected, generated contract 3 passed, pricing verified and `ready_for_catalog`. The follow-up alignment moves the Modal builder allowlist, Worker `OMO_BUILDER_BASE_REVISION`, and exact regression assertion together to `b8419d7…`; local gates are 351 host and 249 Worker passed, syntax/diff clean, with independent review and PR/CI/merge still required before production deployment. The submission remains safely failed after exact dispatch `dispatch_c0460fbd01fe72ba81695c3fa118e6d1`; no further retry occurred. The public static loader activation awaits the coordinator-owned push. Japanese Style Story Video's regenerated app separately awaits its exact live-change approval; Stripe LIVE runs on the shared PhonicsMaker Stripe account; a live sk was exposed in chat (rotation pending); the exact active $0.99 PhonicsMaker book slug is not identified and the local magic-link release is not deployed; existing signed-in account for a payment canary; the shared `omo-llm-runner` and Tier-1 runner; paid download fulfillment.
