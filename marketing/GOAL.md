...
- Creator builder infrastructure: issue #63 moved creator-build dispatch off Hetzner through Cloudflare cron -> protected Modal endpoint -> Worker-authoritative atomic claim -> one revision/source-bound ephemeral Hermes build. Issues #65/#67/#69 added recovery, safe failure stages, and immutable-checkout imports. PR #72's approvals-off approach was contained and reverted by PR #73. Issue #71/PR #74 now enforce OS-UID and fresh-checkout privilege separation: untrusted Hermes has file/skills only and no release credentials; the trusted fixed-repo/base adapter prepares only issue/branch/PR evidence. Exact revision `617cad1` is deployed to Modal. Issue #75 adds the missing owner retry for pre-runtime canary failures without relaxing other retry states; Worker contracts pass 187/187 and live canary retry remains required.
