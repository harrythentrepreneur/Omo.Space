...
- Next: (1) coordinator pushes the local `SHIPPED-13-READY` release commit and verifies the 13 production cards plus deployed Worker registry; (2) coordinator activates the preserved builder listing separately; (3) re-run an authenticated top-up + Woven checkout canary to confirm the live payment loop; (4) move Omo onto its own Stripe account; (5) rotate the live sk exposed in chat; (6) build the magic-link free-book grant; (7) obtain Harry's explicit approval before any pilot email send.
- Blockers: the local release is complete and awaiting the coordinator-owned push; Stripe LIVE runs on the shared PhonicsMaker Stripe account (account-level branding off-limits; conflicts with a PhonicsMaker sale); a live sk was exposed in chat (rotation pending); Japanese Style Story Video is audio fail-closed (hosted run pinned to the bundled 10s sample); magic-link free-book grant not built; existing signed-in account for a payment canary; the shared `omo-llm-runner` and Tier-1 runner; paid download fulfillment.
...
Hand the local `SHIPPED-13-READY` commit to the coordinator for push, production
13-card/Worker-registry verification, and separate activation of the preserved
`skill-md-to-hosted-workflow` listing. Do not push from this profile.
