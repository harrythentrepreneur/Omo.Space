...
- Education hosting gate: Gate 1R2 regenerated the nine 0.2.2 runtimes with deterministic digraph spans, false-flag stripping, and phoneme-containment filtering; the unchanged 27 real-provider cases reached 27/27 schema-valid and 23/27 semantic at USD 0.00491694, every tool earned at least one semantic pass, and authenticated proxy preflight was 2/2 HTTP 200. The >=24/27 semantic release threshold missed by one, so Modal deployment, marketplace activation, and hosted canaries were not attempted. Every case and exact failure is recorded in `research/one-shot-rounds.md`.
...
- Next: (1) add deterministic decodable target-word filtering, weak word-list evidence filtering, and syllable spelling/separator semantic validation, then rerun the unchanged 27-case provider gate; (2) push the two-listing visibility commit live; (3) re-run an authenticated top-up + Woven checkout canary to confirm the live loop; (4) move Omo onto its own Stripe account (LIVE is currently on the shared PhonicsMaker account); (5) rotate the live sk exposed in chat; (6) decide Japanese Style Story Video (audio fail-closed) vs Facebook Ads Copywriter for the second visible listing; (7) build the magic-link free-book grant; (8) obtain Harry's explicit approval before any pilot email send.
- Blockers: the education gate is 23/27 semantic, one below release: decodable #2 retained one mislabeled target word, phonics-list #3 retained weak/off-topic evidence after its bounded retry, and syllable #2/#3 ignored requested separators while #3 also altered spelling; Stripe LIVE runs on the shared PhonicsMaker Stripe account (account-level branding off-limits; conflicts with a PhonicsMaker sale); a live sk was exposed in chat (rotation pending); Japanese Style Story Video is audio fail-closed (hosted run pinned to the bundled 10s sample); magic-link free-book grant not built; existing signed-in account for an authenticated canary; the shared `omo-llm-runner` and Tier-1 runner; chargeable activation/provider canaries; paid download fulfillment.
...
Implement profile-driven decodable target-word filtering, weak word-list
evidence filtering, and syllable word/order/spelling/separator validation before
the single retry. Re-run the exact same 27 provider cases. Do not deploy or
activate the education wave until every tool has at least one semantic pass and
the total reaches at least 24/27.
