...
- Creator builder infrastructure: issue #63 moved creator-build dispatch off Hetzner through Cloudflare cron -> protected Modal endpoint -> Worker-authoritative atomic claim -> one revision/source-bound ephemeral Hermes build. Issues #65/#67/#69 added recovery, safe failure stages, and immutable-checkout imports. PR #72's approvals-off approach was contained and reverted by PR #73. Issue #71/PR #74 enforce OS-UID and fresh-checkout privilege separation: untrusted Hermes has file/skills only and no release credentials; the trusted adapter prepares only issue/branch/PR evidence. Issue #75/PR #76 added owner retry for pre-runtime canary failures. Issue #77 fixes the reproduced Hermes startup failure by pinning its missing `anthropic==0.87.0` compatibility dependency; isolated Modal proof returns 0 under UID 10001 with the real OpenCode Go provider, and contracts pass 66 builder/queue plus 187 Worker tests.
