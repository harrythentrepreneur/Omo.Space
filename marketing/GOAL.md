...
- Education hosting gate: Brief AG's fresh 27-case provider run reached OpenCode Go but returned `LLM_HTTP_400` for all 27 strict `json_schema` requests, so the acceptance gate is 0/27 and stopped before proxy preflight, Modal deployment, marketplace registration, activation, or hosted canaries. Exact cases and cost availability are recorded in `research/one-shot-rounds.md`.
