...
- THIRD DIFFERENTIAL PORT DONE (2026-08-17): `decodable-sentence-creator` now has a source-derived fixture and four tests. Source contract: camelCase `phonicsPattern`, `numSentences`, `sentenceLength`, `includeSightWords` with 16 exact pattern options and Markdown-string output; current Omo candidate uses snake_case, Boolean sight-word control, adds `dialect`, and returns structured JSON. Evidence `research/parity-evidence/decodable-sentence-creator-drift.json` is `DRIFT_CONFIRMED`: 18 input + 7 output mismatches. Full parity harness passes 17/17.
- NEXT PORT after this: 5 of 8 configured tools with matching Omo containers remain unported (grapheme-to-phoneme-converter, phoneme-counter, phonics-rule-explainer, story-idea-generator, syllable-splitter-and-counter). Continuation is fully loop-owned (offline, no Harry).
